...
- 共享用户 Skills（`~/.agents/skills`）、旧版 Codex Skills、Plugins 和生成图片
- 与选中内容相关的本地状态和路径映射

项目文件和对话历史是两类内容。只选对话时，项目源码不会自动被带走；选中项目时，默认会连同其子对话一起选择，但仍可以手动取消某条对话。

共享用户 Skills 会与旧版 `$CODEX_HOME/skills` 分开展示。共享 Skill 按整个目录迁移：目标端同名且内容不同默认保留目标版本，也可以逐项选择“使用迁移包”；ReHome 不会把两个目录逐文件混合。安全且受支持的 `skills` CLI v3 lock 条目会随目录决策合并，损坏或未知版本的目标 lock 保持不变。
...
共享 Skill 内发现 symlink、reparse point、特殊文件、敏感认证文件或高置信度凭据内容时，ReHome 会阻止整个 Skill 打包，只显示文件路径和原因，不显示凭据值。

## 需要知道的限制

这不是官方云同步，也不会让两台电脑每天自动保持一致。跨系统后，旧对话通常仍可作为历史上下文，但它原来绑定的工作目录可能不能继续直接使用。恢复完成后请重新打开项目；需要继续工作时，在恢复后的项目里开一个新任务最稳妥。

当前单个 `.rehome` 包及单个文件最多支持 16 GiB；大文件在创建、检查和恢复时采用流式处理，不会整份读入内存。Codex 线程元数据单独支持到 64 MiB。

登录状态、浏览器会话、正在运行的终端、未保存内容和系统原生依赖不会完整迁移。使用不同账号或 workspace 时，外部服务可能需要重新登录或授权。

ReHome 只迁移 Skill 内容，不安装 Node.js、Python、Git Bash/WSL、外部 CLI、模型、凭据或 API Key。包含 macOS 脚本或本机依赖的 Skill 即使哈希一致并被 Codex 发现，也可能仍需在 Windows 上单独配置运行环境。
